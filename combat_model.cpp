...
{
    float to_hit_prob = clamp(base_hit_probability + probability_modification, 0.f, 1.f);

    bool direct_hit = rand_det_s(rng, 0.f, 1.f) < to_hit_prob;

    bool graze_hit = false;

    if(!direct_hit)
    {
        graze_hit = rand_det_s(rng, 0.f, 1.f) < to_hit_prob;
    }
...
creature_model default_alien_model()
{
    ...
    leg.base_hit_probability = -0.4;

    body_part head;
    head.type = body_part_info::HEAD;
    head.base_hit_probability = -0.7;

    body_part torso;
    torso.max_hp = 4;
    torso.hp = 4;
    torso.type = body_part_info::TORSO;
    torso.base_hit_probability = -0.1;
